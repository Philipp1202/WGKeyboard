...
def main():
    ...
    try:
        f = open(os.path.dirname(pathlib.Path().resolve()) + "/sokgraph_" + layout + ".txt", "r+", encoding="utf-8")  # delete content of file and write new content in it
    except:
        f = open(os.path.dirname(pathlib.Path().resolve()) + "/sokgraph_" + layout + ".txt", "a", encoding="utf-8")   # create file for layout, if there isn't one
